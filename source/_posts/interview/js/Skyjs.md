...
​	Event Loop事件循环机制是 JS 处理异步操作的核心机制，它通过调用栈、任务队列和事件循环的协同工作，解决了单线程语言的”同步阻塞“问题。JS 在执行时会将所有任务分为异步任务和同步任务两种，遇到同步任务会直接放入调用栈（Call Stack）中执行；遇到异步任务则会先交给浏览器的相关API进行处理，自身继续向下执行。浏览器将异步任务处理完成后，会将任务对应的回调函数放到任务队列（Task Queue）中，等待调用栈空闲时再执行，任务队列遵循先进先出（FIFO）原则。

​	任务队列又可细分为微任务队列和宏任务队列，其中微任务的优先级高于宏任务，会被优先执行。

### ① 执行同步任务

​    JS 开始执行（通常从 `<script>` 开始），遇到同步任务，会使用调用栈优先执行，直到调用栈为空。

​    如果遇到了异步任务，则先交给浏览器的相关API进行处理，自身继续向下执行。待浏览器将异步任务处理完成后，会将任务对应的回调函数按照类型放到对应的微任务队列或宏任务队列中。

### ② 执行微任务

​    调用栈空闲后，会立即判断微任务队列中是否有任务，如果有，则将微任务队列中所有任务按照顺序依次推入到调用栈中执行，直到微任务队列清空。

​    如果这个微任务执行过程中，产生了新的微任务，则会直接加入到微任务队列中，等待本次一并执行。直到微任务队列彻底清空，本次执行才会结束；如果产生了新的宏任务，则会将其加入到宏任务队列中等待执行。

### 补充：UI渲染（可选）

​    由于 JS 线程和渲染线程是互斥的，JS 线程执行时，渲染线程会被挂起；渲染线程工作时，JS 线程会被阻塞。因此在微任务全部执行完毕、下个宏任务开始之前，浏览器通常会先执行一次UI渲染（DOM更新、重排、重绘）。

​    但并非绝对执行，因为浏览器会根据屏幕刷新率和性能优化策略调整渲染时机，比如：短时间内有大量宏任务/微任务，就可能会延迟渲染，合并多次渲染操作，避免频繁渲染。

### ③ 执行宏任务

​    在微任务队列清空后，会判断宏任务队列中是否有任务，如果有，则取出队列中第一个宏任务，推入到调用栈中执行，直到执行完毕。

​    如果这个宏任务执行过程中，产生了微任务，则会加入到微任务队列中等待执行；如果产生了新的宏任务，则会加入到宏任务队列末尾等待执行。

### ④ 重复循环

​    回到第②步，进入下一轮，并循环执行第②~③步。

### 2、上面提到了微任务和宏任务，请展开讲讲？

#### ① 微任务

​    微任务执行优先级高于宏任务，且每轮事件循环会依次将队列中的所有微任务执行完毕，包括本次执行中产生的新的微任务。

​    微任务的设计初衷是为了在当前宏任务执行结束之后，浏览器渲染之前，下一个宏任务开始之前，插入一个高优先级的执行时机，从而能够尽快响应状态变化。

#### **常见的微任务：**

- Promise().then/catch/finally，但要注意 Promise(function{ }) 中的代码属于同步任务。
- async/await，本质是 Promise 的语法糖。
- MutationObserver，浏览器专属，用于监听DOM变化。
- process.nextTick，Node.js专属，在微任务中的执行优先级最高。
- Window.queueMicrotask()方法添加的微任务，浏览器专属。

**注意：**

​    Node.js特有的 process.nextTick() ，在ESM模块和CJS模块中的执行优先级在不同场景下有所不同。在CJS模块中，process.nextTick() 永远是同轮次微任务中执行优先级最高的，会被单独插入到一个nextTick队列中。在执行微任务时，会优先执行该队列，然后再执行其他微任务；而在ESM模块中， process.nextTick() 也会被单独插入到一个nextTick队列中，但如果其位于模块顶层，则是同轮次微任务中执行优先级最低的，会优先执行其他微任务，然后再执行该队列。如果位于模块非顶层（函数内部等），则其逻辑与CJS模块一致，依然是同轮次微任务中执行优先级最高的，会优先执行。

​    ESM 之所以如此设计，是因为ESM模块的异步加载机制，其模块被加载时，本身会被包进一个 Promise 中，当模块存在依赖时，依赖模块也会被异步加载，加载过程中也可能会产生微任务。为了确保模块依赖能正确解析，所以会先执行其他微任务，然后再执行顶层产生的nextTick队列；而在非顶层场景（如函数内），模块加载已完成，无需考虑依赖解析，因此恢复为 CJS 的优先逻辑。

#### ② 宏任务

​    宏任务执行优先级低于微任务，且每轮事件循环只会执行一个宏任务。

#### **常见的宏任务：**

- `<script>` 标签内的整体代码，视为第一个宏任务。
- 定时器 setTimeout/setInterval。
- I/O操作，如文件读取、网络请求等。
- DOM 事件，浏览器专属，如click、focus等。
- setImmediate，Node.js专属，用于执行回调。

### 3、Node.js与浏览器中的Event Loop机制有什么不同？

​    Node.js作为服务端 JS 运行环境，其Event Loop机制是基于libuv库实现的，虽核心理念与浏览器相同，但在具体实现上有一定差异，主要体现在宏任务和微任务的执行逻辑。Node.js中的 Event Loop 将宏任务队列分为六个阶段，按固定顺序执行，每个阶段都有对应的任务队列，不同的任务会被放入不同阶段的任务队列中等待执行。如果当前阶段的宏任务执行过程中触发了新的宏任务，且该宏任务也属于当前阶段，则该任务也会在本次一并执行，只有当前阶段的任务全部执行完毕或到达上限后，才会进入下个阶段。 六个阶段具体是：

#### ① timers

​    该阶段用于处理已经到达时间的 setTimeout、setInterval的回调。

#### ② pending callbacks

​    该阶段用于处理系统 I/O 回调，比如TCP连接错误的回调等。

#### ③ idle/prepare

​    该阶段用于libuv 内部使用，idle为闲置阶段，prepare为准备阶段。

#### ④ poll

​    该阶段用于处理 I/O 回调，如文件读取、网络请求等，并检索新的 I/O 事件。

​    该阶段的任务队列处理清空后，Node.js会根据具体情况决定是否阻塞等待：如果 Check 阶段的任务队列存在任务，或着 timers 阶段的任务队列存在任务，则不阻塞，立即进入下个阶段；如果Check 阶段和 timers 阶段不存在任务，则会短暂的阻塞检索并处理新的I/O事件。如果阻塞超时或检索无新事件，则也会进入下个阶段。该机制通过短暂等待，避免了频繁切换阶段，减少了切换开销，提升了处理吞吐量，是优化高并发场景性能的关键点之一。

#### ⑤ check

​    该阶段只用于处理 setImmediate 的回调。

#### ⑥ close callbacks

​    该阶段用于处理关闭相关的回调，如websocket的close等。



而微任务队列的执行时机，则是根据Node版本不同，分为两种情况：

#### ① v11 之前

​    在Node.js v11 之前，微任务会在两个节点执行，首先是在主线程执行完同步代码之后，会先执行清空微任务队列，再进入宏任务的第一个阶段；其次是在这六个阶段中每个阶段执行结束后，下个阶段开始前，会先执行清空微任务队列，然后再进入下个阶段。

#### ② v11 及之后

​    在Node.js v11及之后，微任务同样会在两个节点执行，首先会在主线程执行完同步代码之后，会先执行清空微任务队列，再进入宏任务的第一个阶段；第二个节点则发生变化，变为在每个阶段的每个宏任务执行结束后，先执行清空微任务队列，然后再执行下一个宏任务。这统一了Node.js与浏览器的行为，减少跨环境开发的复杂度。
